...
const fmt12 = (h, m) => {
  const hh = Math.floor(h); const mm = Math.round(m || 0)
  return `${hh>12?hh-12:hh===0?12:hh}:${pad(mm)} ${hh>=12?'PM':'AM'}`
}

/* ── Parse ISO datetime string → "H:MM AM/PM" ── */
function parseDT(iso) {
  if (!iso) return null
  try {
    // Prokerala returns e.g. "2026-04-11T06:10:00+05:30"
    const d = new Date(iso)
    return fmt12(d.getHours(), d.getMinutes())
  } catch { return null }
}

/* ── Parse Prokerala panchang response ── */
function parseProkeralaResponse(raw, city) {
  try {
    // Prokerala v2 structure: raw.data.panchang  OR  raw.panchang
    const p = raw?.data?.panchang || raw?.panchang || raw?.data || raw
    const cityConf = CITIES[city]
    const now = new Date()

    // Helper: extract name from array of objects
    const getName = (arr) => {
      if (!arr) return null
      if (Array.isArray(arr)) return arr[0]?.name || arr[0]?.id || null
      if (typeof arr === 'string') return arr
      return arr?.name || null
    }

    // Rahu Kalam — compute from day of week
    const rahuMap = { 0:[17,18], 1:[7.5,9], 2:[15,16.5], 3:[12,13.5], 4:[13.5,15], 5:[10.5,12], 6:[9,10.5] }
    const [rahuStart, rahuEnd] = rahuMap[now.getDay()]
    const rahuStr = `${fmt12(Math.floor(rahuStart), (rahuStart%1)*60)} – ${fmt12(Math.floor(rahuEnd), (rahuEnd%1)*60)}`

    // Extract sunrise/sunset
    const sunrise = parseDT(p?.sunrise) || parseDT(p?.sun_rise) || fmt12(6, 10)
    const sunset  = parseDT(p?.sunset)  || parseDT(p?.sun_set)  || fmt12(18, 25)
    const moonrise = parseDT(p?.moonrise) || parseDT(p?.moon_rise) || fmt12(14, 30)

    // Rahu kalam from API if available
    const rahuKalam = parseDT(p?.rahu_kaal?.start) && parseDT(p?.rahu_kaal?.end)
      ? `${parseDT(p.rahu_kaal.start)} – ${parseDT(p.rahu_kaal.end)}`
      : parseDT(p?.rahu_kalam?.start) && parseDT(p?.rahu_kalam?.end)
      ? `${parseDT(p.rahu_kalam.start)} – ${parseDT(p.rahu_kalam.end)}`
      : rahuStr

    const yamaGandam = parseDT(p?.yama_gandam?.start) && parseDT(p?.yama_gandam?.end)
      ? `${parseDT(p.yama_gandam.start)} – ${parseDT(p.yama_gandam.end)}`
      : `${fmt12(10, 30)} – ${fmt12(12, 0)}`

    const abhijit = parseDT(p?.abhijit_muhurta?.start) && parseDT(p?.abhijit_muhurta?.end)
      ? `${parseDT(p.abhijit_muhurta.start)} – ${parseDT(p.abhijit_muhurta.end)}`
      : `${fmt12(11, 48)} – ${fmt12(12, 36)}`

    return {
      date: now.toLocaleDateString('en-IN', {weekday:'long', day:'numeric', month:'long', year:'numeric'}),
      tithi:          getName(p?.tithi)     || getName(p?.tithi_details),
      nakshatra:      getName(p?.nakshatra) || getName(p?.nakshatra_details),
      yoga:           getName(p?.yoga)      || getName(p?.yoga_details),
      karana:         getName(p?.karana)    || getName(p?.karana_details),
      sunrise, sunset, moonrise,
      rahuKalam, yamaGandam, abhijitMuhurta: abhijit,
      city: cityConf.name, cached: false,
...
/* ── Generate computed demo panchang (fallback) ── */
function generateDemoPanchang(city) {
  const cityConf = CITIES[city]
  const now = new Date()
  ...
  return {
    ...
    sunrise:   fmt12(srH, srM), sunset: fmt12(ssH, ssM), moonrise: fmt12(srH+8, (srM+20)%60),
    ...
    city: cityConf.name, cached: false, updatedAt: new Date().toISOString(),
  }
}

/* ── Main fetchPanchang: Firestore cache → Cloud Run → Demo ── */
const CLOUD_RUN_URL = 'https://getpanchang-pchldvlwca-el.a.run.app'
...
async function fetchPanchang(city) {
  ...
  try {
    ...
    if (snap.exists() && snap.data()[city]) {
      console.log('[Panchang] Firestore cache hit:', city)
      return { ...snap.data()[city], cached: true }
    }
  } catch(e) { console.warn('[Panchang] Firestore read failed:', e.message) }

  // 2. Call your deployed Cloud Run function
  try {
    console.log('[Panchang] Calling Cloud Run for:', city)
    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), 8000)
    const response = await fetch(`${CLOUD_RUN_URL}?city=${city}`, {
    ...
    console.log('[Panchang] Cloud Run raw response keys:', Object.keys(raw || {}))

    // Parse the Prokerala nested response
    const parsed = parseProkeralaResponse(raw, city)
    if (!parsed) throw new Error('Parse failed')

    // Save to Firestore for next users
    try {
      const existing = (await getDoc(ref)).data() || {}
      await setDoc(ref, { ...existing, [city]: parsed, updatedAt: serverTimestamp() }, { merge: true })
    } catch(e) { console.warn('[Panchang] Firestore write failed:', e.message) }

    return parsed

  } catch(e) {
    console.warn('[Panchang] Cloud Run failed:', e.message, '— using demo data')
  }

  // 3. Final fallback — computed demo data
  const demo = generateDemoPanchang(city)
  return demo
}

/* ── Loading shimmer ── */
function Shimmer({ w='100%', h=14, r=8 }) {
  return <div style={{ width:w, height:h, borderRadius:r, background:'linear-gradient(90deg,rgba(255,255,255,0.08) 25%,rgba(255,255,255,0.18) 50%,rgba(255,255,255,0.08) 75%)', backgroundSize:'400% 100%', animation:'shimmer 1.5s ease-in-out infinite' }} />
}
...
function PRow({ label, value, accent, big, highlight }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'9px 0', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
      <span style={{ fontSize:11, fontWeight:600, color:'rgba(255,255,255,0.65)', textTransform:'uppercase', letterSpacing:'0.1em', fontFamily:'Poppins,sans-serif' }}>{label}</span>
...
function PanchangCard({ currentUser }) {
  ...
  return (
    <GlassCard accent="rgba(212,175,55,0.5)" glow="#d4af37" style={{ marginBottom:14, overflow:'hidden' }}>
      {/* Gold header */}
      <div style={{ padding:'14px 16px 12px', borderBottom:'1px solid rgba(212,175,55,0.3)', background:'linear-gradient(90deg,rgba(212,175,55,0.18),rgba(212,175,55,0.04))' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          {/* City selector */}
          <select value={city} onChange={e=>setCity(e.target.value)} style={{ padding:'6px 10px', background:'rgba(255,255,255,0.15)', border:'1.5px solid rgba(212,175,55,0.5)', borderRadius:10, color:'#f0f0f0', fontSize:12, fontFamily:'Poppins,sans-serif', fontWeight:700, outline:'none', cursor:'pointer' }}>
          ...
          <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:12 }}>
              {[
                ...
                <div key={i} style={{ textAlign:'center', padding:'10px 6px', background:'rgba(255,255,255,0.09)', borderRadius:14, border:`1px solid ${s.color}25` }}>
                  ...
                  <p style={{ fontSize:9, color:'rgba(255,255,255,0.65)', margin:0, textTransform:'uppercase', letterSpacing:'0.1em', fontFamily:'Poppins,sans-serif' }}>{s.label}</p>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginTop:10, padding:'10px 14px', background:'rgba(239,68,68,0.18)', border:'1.5px solid rgba(239,68,68,0.45)', borderRadius:12, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize:9, color:'rgba(255,255,255,0.6)', margin:0, fontFamily:'Poppins,sans-serif' }}>Avoid important work</p>
                </div>
              </div>
            ...
            {/* Yama Gandam */}
            <div style={{ marginTop:8, padding:'8px 14px', background:'rgba(251,191,36,0.15)', border:'1.5px solid rgba(251,191,36,0.35)', borderRadius:12, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
      ...
      <div style={{ padding:'14px 16px 12px', background:`linear-gradient(135deg,${zodiac.color}18,transparent)`, borderBottom:`1px solid ${zodiac.color}30` }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <div style={{ textAlign:'right' }}>
            <p style={{ fontSize:9, color:'rgba(255,255,255,0.6)', margin:0, fontFamily:'Poppins,sans-serif' }}>{new Date().toLocaleDateString('en-IN',{day:'numeric',month:'short'})}</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:'12px 16px 14px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:7 }}>
          {[
            ...
            <div key={i} style={{ textAlign:'center', padding:'8px 6px', background:'rgba(255,255,255,0.08)', borderRadius:12, border:'1px solid rgba(255,255,255,0.14)' }}>
              <p style={{ fontSize:9, color:'rgba(255,255,255,0.7)', textTransform:'uppercase', letterSpacing:'0.08em', margin:'0 0 6px', fontFamily:'Poppins,sans-serif' }}>{bar.label}</p>
              <div style={{ height:4, borderRadius:4, background:'rgba(255,255,255,0.15)', overflow:'hidden', marginBottom:4 }}>
...
function CosmicInsightCard({ insights, isThinking, onGenerate, zodiac }) {
  const { t } = useLanguage()
  const cards = [
    { icon:'🎨', label:t.balance ? 'Lucky Color' : 'Lucky Color',        color:'#a78bfa', idx:0 },
    ...
    <GlassCard accent="rgba(167,139,250,0.4)" glow="#7c3aed" style={{ marginBottom:14 }}>
      <div style={{ padding:'14px 16px 12px', borderBottom:'1px solid rgba(167,139,250,0.15)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <button onClick={onGenerate} disabled={isThinking} style={{ padding:'7px 14px', borderRadius:10, border:'none', cursor:isThinking?'not-allowed':'pointer', background:isThinking?'rgba(124,58,237,0.2)':'linear-gradient(135deg,#7c3aed,#4f46e5)', color:'#fff', fontSize:11, fontWeight:700, fontFamily:'Poppins,sans-serif', opacity:isThinking?0.7:1, display:'flex', alignItems:'center', gap:6 }}>
          {isThinking ? <><div style={{ width:10, height:10, border:'2px solid rgba(255,255,255,0.25)', borderTop:'2px solid #fff', borderRadius:'50%', animation:'spin 0.7s linear infinite' }} />{t.thinking||'Reading…'}</> : t.generate||'✨ Generate'}
        </button>
      </div>
      <div style={{ padding:'12px 16px 14px', display:'flex', flexDirection:'column', gap:9 }}>
        {cards.map((c,i)=>(
          <div key={i} style={{ display:'flex', gap:11, padding:'11px 13px', background:`${c.color}18`, borderRadius:14, border:`1px solid ${c.color}20` }}>
...
export default function Astro(props) {
  ...
  return (
    ...
    <style>{`
      ...
      .astro-page-wrap { background:linear-gradient(160deg,#06030f 0%,#0d0620 40%,#050218 100%) !important; min-height:100vh; }
      ...
      input::placeholder{color:rgba(255,255,255,0.4)!important;} .astro input{color:#fff!important;}
      input[type=date]::-webkit-calendar-picker-indicator{filter:invert(0.6);}
      input[type=time]::-webkit-calendar-picker-indicator{filter:invert(0.6);}
      select option{background:#1a0a2e!important;}
    `}</style>

    <div className="astro-page-wrap"><div className="astro" style={{ maxWidth:860, margin:'0 auto', paddingBottom:60, position:'relative' }}>

      {/* ── HEADER ── */}
      <div style={{ position:'relative', overflow:'hidden', borderRadius:20, marginBottom:12, padding:'16px 18px', background:'linear-gradient(135deg,rgba(124,58,237,0.35),rgba(79,70,229,0.2),rgba(5,3,15,0.95))', border:'1px solid rgba(167,139,250,0.35)', boxShadow:'0 8px 32px rgba(0,0,0,0.4)', animation:'slideUp 0.4s ease-out both' }}>
        <GalaxyCanvas />
        ...
        <div style={{ animation:'fadeIn 0.5s ease-out both' }}>
          <GlassCard accent="rgba(167,139,250,0.3)" glow="#7c3aed" style={{ overflow:'hidden', position:'relative' }}>
            ...
            <div style={{ position:'relative', zIndex:2, padding:'24px 20px' }}>
              ...
              {/* Form */}
              <div style={{ background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.14)', borderRadius:16, padding:16, maxWidth:480, margin:'0 auto' }}>
                <div style={{ marginBottom:12 }}>
                  <label style={{ display:'block', fontSize:9, fontWeight:700, color:'rgba(255,255,255,0.7)', textTransform:'uppercase', letterSpacing:'0.14em', marginBottom:6 }}>Full Name</label>
                  <input value={astroProfile?.name||''} onChange={e=>setAstroProfile({...astroProfile,name:e.target.value})} placeholder="e.g. Ashwin Kumar" style={{ width:'100%', padding:'11px 14px', background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', borderRadius:12, color:'#ffffff', fontSize:14, outline:'none', fontFamily:'Poppins,sans-serif' }} />
                </div>
                <div style={{ display:'flex', gap:10, marginBottom:12 }}>
                  <div style={{ flex:1 }}>
                    <label style={{ display:'block', fontSize:9, fontWeight:700, color:'rgba(255,255,255,0.7)', textTransform:'uppercase', letterSpacing:'0.14em', marginBottom:6 }}>Date of Birth</label>
                    <input type="date" value={astroProfile?.dob||''} onChange={e=>setAstroProfile({...astroProfile,dob:e.target.value})} style={{ width:'100%', padding:'11px 14px', background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', borderRadius:12, color:'#ffffff', fontSize:14, outline:'none', fontFamily:'Poppins,sans-serif', colorScheme:'dark' }} />
                  </div>
                  <div style={{ flex:1 }}>
                    <label style={{ display:'block', fontSize:9, fontWeight:700, color:'rgba(255,255,255,0.7)', textTransform:'uppercase', letterSpacing:'0.14em', marginBottom:6 }}>Birth Time</label>
                    <input type="time" value={astroProfile?.time||''} onChange={e=>setAstroProfile({...astroProfile,time:e.target.value})} style={{ width:'100%', padding:'11px 14px', background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', borderRadius:12, color:'#ffffff', fontSize:14, outline:'none', fontFamily:'Poppins,sans-serif', colorScheme:'dark' }} />
                  </div>
                </div>
                <div style={{ marginBottom:16 }}>
                  <label style={{ display:'block', fontSize:9, fontWeight:700, color:'rgba(255,255,255,0.7)', textTransform:'uppercase', letterSpacing:'0.14em', marginBottom:6 }}>Location</label>
                  <input value={astroProfile?.location||''} onChange={e=>setAstroProfile({...astroProfile,location:e.target.value})} placeholder="e.g. Chennai, Tamil Nadu" style={{ width:'100%', padding:'11px 14px', background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', borderRadius:12, color:'#ffffff', fontSize:14, outline:'none', fontFamily:'Poppins,sans-serif' }} />
                </div>
        ...
        <div style={{ animation:'fadeIn 0.4s ease-out both' }}>
            ...
            <div style={{ position:'relative', zIndex:2 }}>
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                ...
                <button onClick={generateAstroData} disabled={isAstroThinking} style={{ padding:'8px 18px', borderRadius:10, background:isAstroThinking?'rgba(124,58,237,0.2)':'linear-gradient(135deg,#7c3aed,#4f46e5)', border:'none', color:'#fff', fontWeight:700, fontSize:12, cursor:'pointer', display:'flex', alignItems:'center', gap:6, fontFamily:'Poppins,sans-serif', boxShadow:isAstroThinking?'none':'0 3px 14px rgba(124,58,237,0.35)' }}>
                  {isAstroThinking?<><div style={{ width:10, height:10, border:'2px solid rgba(255,255,255,0.25)', borderTop:'2px solid #fff', borderRadius:'50%', animation:'spin 0.7s linear infinite' }} />Reading Stars…</>:<>🔮 Daily Reading</>}
                </button>
              </div>
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))', gap:8, marginBottom:12, animation:'slideUp 0.4s ease-out 0.1s both' }}>
            {[
              ...
              <div key={i} style={{ background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.15)', borderRadius:14, padding:'12px 12px', animation:`slideUp 0.35s ease-out ${i*60}ms both` }}>
          ...
          {/* About panel */}
          <GlassCard style={{ padding:'14px 16px', background:'linear-gradient(145deg,rgba(255,255,255,0.1),rgba(255,255,255,0.06))' }}>
